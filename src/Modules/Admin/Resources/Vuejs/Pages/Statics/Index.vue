<template>
  <div>
    ...
    <div class="flex items-center justify-between mb-6">
      ...
      <Link class="btn-indigo" :href="route(routeName+'.create')">
      ...
      <div class="bg-white rounded-md shadow">
        ...
        <tr v-for="(item,id) in listings.data" :key="item.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          ...
          <td class="border-t ">
            ...
            <td class="border-t ">
                <Link class="flex items-center px-6 py-4 focus:text-indigo-500" :href="route('static',item.slug)">
                    {{ item.slug }}
                    <icon v-if="item.deleted_at" name="trash" class="flex-shrink-0 ml-2 w-3 h-3 fill-gray-400" />
                </Link>
            </td>
          <td class="border-t">
            ...
            <td class="border-t text-center">
                <Link class="flex items-center px-6 py-4" @click="destroy(item.id)" tabindex="-1">
  ...
  watch: {
    form: {
      ...
      handler: throttle(function () {
        this.$inertia.get(route(this.routeName), pickBy(this.form), { preserveState: true })
      }, 150),
    },
  },
  methods: {
